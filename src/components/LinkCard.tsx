  ...
  return (
    <article
      className={cn(
        ...
        viewMode === "grid" ? "flex flex-col rounded-xl items-start" : "flex items-center gap-4 p-4 rounded-xl",
    ...
    >
      ...
      {viewMode === "grid" && link.image && (
        <div className="w-full h-32 overflow-hidden border-b border-border/40 bg-secondary/15 shrink-0 relative">
          <img
            src={link.image}
            alt=""
      ...
      <div className={cn(
        "flex-1 min-w-0 w-full flex gap-3",
        viewMode === "grid" ? "flex-col p-4" : "items-center"
      )}>
        <div className="flex gap-3 items-center w-full">
          {dragHandle && (
            <div className="text-text-faint hover:text-text-muted cursor-grab active:cursor-grabbing transition-colors shrink-0">
              {dragHandle}
            </div>
          )}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              ...
              {link.isPinned && (
                <span className="material-symbols-outlined text-[14px] text-accent font-bold" style={{ fontVariationSettings: "'FILL' 1" }}>pushpin</span>
              )}
              {hasReminder && (
      ...
      <div className={cn(
        ...
        {link.password && (
          <button
            ...
            onClick={async (e) => { 
              e.preventDefault(); 
              await copyPasswordAndScheduleClear(link.password || "");
            }}
            className="p-1.5 rounded-md text-text-muted hover:text-text hover:bg-surface-2 transition-colors"
            title="Copy Password"
          >
        ...
        {onReadMode && (
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); onReadMode(); }}
            className="p-1.5 rounded-md text-text-muted hover:text-text hover:bg-surface-2 transition-colors"
